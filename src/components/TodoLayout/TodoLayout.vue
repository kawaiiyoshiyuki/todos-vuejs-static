<template>
  <section>
    ...
    />
    <div class="truncate">
      Add your todo task above, hit Enter to submit.
    </div>
    <ul class="collection" v-for="todo in todos">
      ...
      >
          {{ todo.text }}
      <button class="todo" type='button' @click="(event) => handleUpdateTodo(todo.id, event)">
        <span v-if="todo.completed"><i class="fa fa-undo" /></span>
        <span v-else><i class="fa fa-check-circle" /></span>
      </button>
      <button class="todo" type='button' @click="(event) => handleDeleteTodo(todo.id, event)">
...
export default {
  ...
  data() {
    return {
      todos: [
        ...
        { id: 1, text: 'Open build and deploy settings and connect', completed: false },
  ...
  methods: {
    add() {
      ...
      this.todos.push({ id: this.todos.length, text: this.value, completed: false })
      this.value = ''
    },
